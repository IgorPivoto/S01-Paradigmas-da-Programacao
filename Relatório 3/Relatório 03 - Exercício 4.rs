...
fn main() {
    ...
    let nota1: f64 = entrada.trim().parse().expect("Número inválido");

    entrada.clear();

    println!("Digite a nota NP2:");
    io::stdin().read_line(&mut entrada).expect("Erro ao ler");
    let nota2: f64 = entrada.trim().parse().expect("Número inválido");

    entrada.clear();
    
    println!("Digite a nota NPL:");
    io::stdin().read_line(&mut entrada).expect("Erro ao ler");
    let nota3: f64 = entrada.trim().parse().expect("Número inválido");
